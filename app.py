...
# Filter the available profiles based on the allowed score types
    ...
    if selected_profile == "Striker":
        ...
        columns = ["Player Name", "xG per 90", "Non-Pen Goals per 90", "Shots per 90", "OBV Shot per 90", "xA per 90", "Dribble & Carry OBV", "PAdj Pressures", "Runs in Behind (ST)", "Threat of Runs in Behind (ST)", "Top 5 PSV-99 Percentile"]
...
# Create a scatter plot using Plotly with the filtered data
    fig = px.scatter(filtered_df, x=x_variable, y=y_variable, hover_data={'Player Name': True, 'team_name':True, x_variable:False, y_variable:False})
